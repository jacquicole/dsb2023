...
payments %>% 
  ...
  filter(year >= 2020) %>%
#  group_by(entity,value,member_id) %>%
  count(entity,value,member_id) %>%
#  summarise(tot=sum(value))
  mutate(percent = 100*value*n/sum(n*value)) %>%
...
all_consulting_jobs <- map_df(page, scrape_jobs)

#Kostis : for some reason this function isn't working - it produces me the message:
...
#! HTTP error 404.
#Backtrace:
#  1. purrr::map_df(page, scrape_jobs)
# 13. xml2:::read_html.default(.)
# 17. xml2:::read_xml.character(...)
# 18. xml2:::read_xml.connection(...)
# 20. base::open.connection(x, "rb")
#> all_consulting_jobs.csv <- map_df(page, scrape_jobs)

# Now I just need to write the output in a csv format re: the question:


#Write the data frame to a csv file called all_consulting_jobs.csv in the data folder.

# Kostis: Once I get the above function working can I complete this last question by simply replacing the last code line above with the following line?
...
## Now adding the CSS selector information that I found above using Right click and `inpect` to find the CSS selector for the information.
...
functionality <- listings_html %>%
  html_elements(css = "#dataTable > tbody > tr:nth-child(1) > th.hide-tablet-and-less") %>%
  html_table()

functionality

jobtype <- listings_html %>%
  html_elements(css = "#dataTable > tbody > tr:nth-child(1) > th.hide-tablet-landscape") %>%
  html_table()
