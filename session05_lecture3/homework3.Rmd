...
#
#      left_join(prop, by = "value") %>%
#    glimpse
  
#    mutate(date  = case_when(
#     date %in% c("Registered") ~ "Registered",
#          TRUE ~ "NA"
#   )) %>%
#  remove(na.rm = TRUE) %>%
#prop <-  mutate(year = str_sub(date, -4)) %>%
#  filter(year >= 2020) %>%
#  count(value) %>%
#  mutate(percent = 100*value*n/sum(n*value)) %>%
#  arrange(desc(percent)) %>%

#        select(entity,percent,member_id)
...
-   Write the data frame to a csv file called `contributions-all.csv` in the `data` folder.
    ...
    contributions <- tibble(    # the first 'job' is the variable name, the second job after the equal sign is what you got from the CSS selector
